...
# -*- coding: utf-8 -*-
"""
Evidence Extractor - Extract and quote evidence supporting compliance findings
Identifies performance data, disclaimers, and specific text passages with location tracking
"""
...
class EvidenceExtractor:
    """
    Extract evidence supporting compliance findings
    
    Identifies:
    - Actual performance data (numbers with percentages)
    - Disclaimers using semantic matching
    - Specific text passages with location tracking
    """
    ...
    def extract_evidence(self, text: str, violation_type: str, location: str = "") -> Evidence:
        """
        ...
        Returns:
            Evidence with quotes, locations, and context
        """
        try:
            quotes = []
            locations = []
            ...
            if violation_type == "performance_data":
                perf_data = self.find_performance_data(text)
                if perf_data:
                ...
                quotes = self._extract_relevant_quotes(text, max_quotes=3)
                ...
                quotes = self._extract_relevant_quotes(text, max_quotes=5)
                locations = [location] if location else []
        ...
        try:
            ...
            for pattern in self.performance_patterns:
                ...
                for match in matches:
                    ...
                    location = self._extract_location_from_context(text, match.start())
                    
                    # Calculate confidence based on context
                    confidence = self._calculate_performance_confidence(context, value)
            ...
            if self.ai_engine and performance_data:
                performance_data = self._enhance_with_ai_analysis(text, performance_data)
            
            # Remove duplicates and sort by confidence
            performance_data = self._deduplicate_performance_data(performance_data)
    ...
    def find_disclaimer(self, text: str, required_disclaimer: str) -> Optional[DisclaimerMatch]:
        ...
        Returns:
            DisclaimerMatch if found, None otherwise
        """
        try:
            # Convert to string if dict/json
            if isinstance(text, dict):
            ...
            if required_lower in text_lower:
                location = self._extract_location_from_context(text, text_lower.index(required_lower))
            ...
            if disclaimer_score >= 2:
                ...
                disclaimer_text = self._extract_disclaimer_text(text, matched_keywords)
                location = self._extract_location_from_context(text, 0)
            ...
            if self.ai_engine:
                ai_match = self._ai_disclaimer_match(text, required_disclaimer)
                if ai_match:
                    return ai_match
    ...
    def _extract_relevant_quotes(self, text: str, max_quotes: int = 3, max_length: int = 150) -> List[str]:
        """
    ...
    def _extract_location_from_context(self, text: str, position: int) -> str:
        """
    ...
    def _calculate_performance_confidence(self, context: str, value: str) -> int:
        """
    ...
    def _extract_disclaimer_text(self, text: str, matched_keywords: List[str]) -> str:
        """
    ...
    def _deduplicate_performance_data(self, performance_data: List[PerformanceData]) -> List[PerformanceData]:
        """
    ...
    def _enhance_with_ai_analysis(self, text: str, performance_data: List[PerformanceData]) -> List[PerformanceData]:
        """
...
{{
  "analysis": [
    ...
    def _ai_disclaimer_match(self, text: str, required_disclaimer: str) -> Optional[DisclaimerMatch]:
        """
...
def extract_all_text_from_doc(doc: Dict) -> str:
    """
    Extract all text from document structure
    ...
    try:
        ...
        for key, value in doc.items():
            if isinstance(value, dict):
                all_text.append(json.dumps(value, ensure_ascii=False))
...
if __name__ == "__main__":
    ...
    print("\n🔍 Testing performance data extraction...")
    perf_data = extractor.find_performance_data(test_text)
    print(f"  Found {len(perf_data)} performance data points:")
    for pd in perf_data:
        print(f"    - {pd.value} (confidence: {pd.confidence}%)")
    ...
    if disclaimer.found:
        print(f"  Similarity score: {disclaimer.similarity_score}%")
